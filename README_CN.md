# PhotoCleaner - 本地照片清理助手 (已归档 / Archived)

> ⚠️ **项目状态：已停止维护与封存归档 (Project Retired & Archived)**
>
> 本项目已于 **2026 年 9 月正式停止维护并转为只读归档**。代码仓库完整保留，作为 Android 现代技术栈（Kotlin 2.4、Jetpack Compose、Room、Hilt、MediaStore SAF）及纯本地离线图片处理架构的探索样板。
>
> ### 📦 项目复盘与归档原因：
> 1. **手机原生系统相册体验碾压**：现代主流移动操作系统（如 Xiaomi HyperOS、Apple iOS 等）已在系统底层深度打通 NPU 硬件加速、相册级回收站及原生垃圾截图/连拍清理机制。无论在处理吞吐率、扫描速度还是日常交互流畅度上，第三方受限于存储沙箱的独立 App 均被降维打击。
> 2. **“100% 离线”与“智能可用”的天然矛盾**：为确保零网络与绝对隐私，应用强依赖本地轻量 ML 模型，但分类粗糙且置信度有限；为了“绝对不误删”，又被迫将海量照片的复核工作甩回给用户逐张确认，导致“操作繁重、收益低下”，偏离了工具应“省心省力”的初衷。
> 3. **工程探索使命圆满完成**：项目完整跑通了 Android 16 (API 37) 兼容、自适应平板/折叠屏布局、分级媒体权限适配、零网络离线打包以及系统级永久删除校验，技术验证目的已完全达到。
